...
- **`AI_TIME_LOG.md` — running estimate, updated during the PR; row added in the PR itself.** The `## AI time log` section in the PR description is the running estimate — bump the numbers as you work, when review feedback adds another round, when scope grows. **The row in `AI_TIME_LOG.md` is part of the deliverable**, not stitched in post-merge: append it to the file as part of the PR. CI enforces this — `scripts/check-ai-time-log-row.mjs` fails any PR that doesn't modify `AI_TIME_LOG.md`. Escape hatch for trivial PRs (typo fix, Dependabot bump, non-author contribution): include `[skip-time-log]` in any commit message in the PR. Calibration over time is the goal; precision isn't.
