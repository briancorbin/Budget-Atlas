...
function makeFilers(
  ...
  const filer = (label: string, gross: number, std: number, brackets: readonly TaxBracket[]): FilerWalk => {
    const taxable = Math.max(0, gross - std);
    const rows = bracketBreakdown(taxable, brackets);
    return {
      label, gross, stdDeduction: std, taxable, rows,
  ...
  if (filing === 'married') {
    return [filer(
      ...
      incomeA + incomeB, stdFor('married'), bracketsFor('married'),
    )];
  }
  ...
  if (cohabitating) {
    return [
      filer(`Primary (filing ${filingLabel[filing]})`, incomeA, stdFor(filing), bracketsFor(filing)),
...
export function BracketWalkthrough({
  result, incomeA, incomeB, hasPartner, filing,
}: {
  result: BudgetResult;
  incomeA: number;
  incomeB: number;
  ...
  const federalFilers = makeFilers(
    incomeA, incomeB, hasPartner, filing,
  ...
  const stateNoTax = stateTotal === 0 && stateFilers.every(f => f.rows.every(r => r.rate === 0));
    ...
    <div style={{ marginTop: 16, marginBottom: 32 }}>
      <button
        ...
        style={{
          ...
          padding: '10px 14px', background: open ? T.bgAlt : T.surface,
      ...
      {open && (
        <div style={{
          ...
          {!stateNoTax && stateFilers.map((f, i) => (
            <FilerSection key={i} filer={f} />
          ))}

          <div style={{
            marginTop: 4, paddingTop: 16, borderTop: `1px dashed ${T.border}`,
            fontSize: 12, color: T.inkMuted, fontFamily: fonts.body, lineHeight: 1.6,
          }}>
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{
        ...
        color: T.inkSoft, fontWeight: 600, marginBottom: 8,
      }}>{filer.label}</div>

      <div style={{
        display: 'flex', flexWrap: 'wrap', gap: 18, marginBottom: 12,
        fontSize: 13, fontFamily: fonts.body, color: T.inkSoft,
      }}>
        <span>Gross <span style={{ fontFamily: fonts.mono, color: T.ink }}>{fmt(filer.gross)}</span></span>
        <span>− Std deduction <span style={{ fontFamily: fonts.mono, color: T.ink }}>{fmt(filer.stdDeduction)}</span></span>
        <span>= Taxable <span style={{ fontFamily: fonts.mono, color: T.ink }}>{fmt(filer.taxable)}</span></span>
      </div>
...
function BracketTable({ rows, total }: { rows: BracketRow[]; total: number }) {
  return (
    <div style={{
      border: `1px solid ${T.border}`, fontFamily: fonts.mono, fontSize: 13,
      background: T.bg,
    }}>
      <div style={{
        display: 'grid', gridTemplateColumns: '2fr 1fr 1.4fr 1.4fr',
        padding: '8px 12px', borderBottom: `1px solid ${T.border}`,
        fontSize: 11, color: T.inkMuted, letterSpacing: '0.08em',
      ...
      {rows.map((r, i) => (
        <div key={i} style={{
          ...
          <span>
            {r.isUserBracket && (
              <span style={{
                color: T.accent, marginRight: 6, fontFamily: fonts.body,
      ...
      <div style={{
        display: 'grid', gridTemplateColumns: '2fr 1fr 1.4fr 1.4fr',
        padding: '10px 12px', borderTop: `2px solid ${T.ink}`,
        fontWeight: 600,
      }}>
        <span style={{ fontFamily: fonts.body, fontSize: 12, letterSpacing: '0.08em' }}>SUBTOTAL</span>
        <span></span>
        <span></span>
...
function CreditsSummary({ raw, ctc, eitc, net }: {
  raw: number; ctc: number; eitc: number; net: number;
}) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        fontSize: 11, fontFamily: fonts.body, color: T.inkMuted,
        letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8,
      }}>Refundable credits</div>
...
function Row({ label, value, positive, bold, accent }: {
  label: string; value: string; positive?: boolean; bold?: boolean; accent?: string;
}) {
  return (
    <div style={{
      ...
      <span style={{
        fontFamily: fonts.body, color: T.inkSoft, fontSize: 13,
