...
export function StatRow({ result }: { result: BudgetResult }) {
  ...
  return (
    <div style={{
      ...
      gap: 1, marginBottom: 32, background: T.border,
    }}>
      <Stat label="Gross / yr" value={fmt(result.grossIncome)}
        sub={fmt(result.grossIncome / 12) + '/mo'} />
      <Stat label="Total taxes" value={fmt(result.totalTaxes)}
        sub={fmtPct(effRate) + ' effective'} />
      <Stat label="Take-home" value={fmt(result.netIncome)}
        sub={fmt(result.monthlyNet) + '/mo'} accent={T.positive} />
      <Stat label="Monthly expenses" value={fmt(result.totalExpenses)}
        sub={`Household of ${result.householdSize}`} />
      <Stat label="Discretionary" value={fmtSigned(result.discretionary)}
...
export function StatusBanner({ result }: { result: BudgetResult }) {
  ...
  return (
    <div style={{
      marginBottom: 36, padding: '18px 24px',
      ...
      {sustainable ? (
        <>
          <strong style={{ color: T.positive }}>Sustainable.</strong>{' '}
          After all essentials, this household has{' '}
          <strong>{fmt(result.discretionary)}/month</strong> ({fmt(result.annualDiscretionary)}/yr)
          {' '}for savings, vacations, retirement, and personal spending.
        </>
      ) : (
        <>
